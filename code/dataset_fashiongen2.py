...
class TextDataset(data.Dataset):
    def __init__(self, data_dir, split='train', base_size=64,
        ...
        # print("filtered_image", self.filtered_image_indexes)
    ...
    def filter_by_categories(self, selected_categories, split):
        ...
        indexes = []
        for i in range(len(file_h5['category'])):
            if file_h5['category'][i] in selected_categories:
                # print("file_h5['category'][i]", file_h5['category'][i])
                indexes.append(i)
        return indexes
    ...
    def load_captions(self, split, selected_categories):
        ...
        caption_count = len(file_h5['DESCRIPTION'])
        ...
        for i in range(caption_count):
            if file_h5['category'][i] in selected_categories:
                ...
                caption = str(file_h5['DESCRIPTION'][i])
    ...
    def get_imgs(self, index):
        ...
        img = Image.fromarray(self.current_h5_file['image'][h5_index].astype('uint8'), 'RGB')
