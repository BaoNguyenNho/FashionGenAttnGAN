...
if __name__ == "__main__":
    ...
    print("dataset n_words and embedding _num", dataset.n_words, dataset.embeddings_num)
    assert dataset
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=batch_size, drop_last=True, shuffle=True, num_workers=int(cfg.WORKERS))

    print("num worker", int(cfg.WORKERS))
    # import h5py
    # has_h5py_object = any(isinstance(data, h5py.Dataset) for data in dataset)

    # if has_h5py_object:
    #     print("Dataset contains h5py object.")
    # else:
    #     print("Dataset does not contain h5py object.")
